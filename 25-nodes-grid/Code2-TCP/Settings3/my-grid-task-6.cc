...
void TraceSink (std::size_t index, Ptr<const Packet> p, const Address& a)
{
  ...
  // std::cout << "Received "<< p->GetSize () << "bytes"<< " at " << Simulator::Now ().GetSeconds () << " from " 
  // << InetSocketAddress::ConvertFrom (a).GetIpv4() << std::endl;
  std::cout << "At " << Simulator::Now ().GetSeconds ()
  			<< " node" << index << " received " << p->GetSize () << "bytes"
  			<< " from "<< InetSocketAddress::ConvertFrom (a).GetIpv4() << std::endl;
}
...
int main (int argc, char *argv[])
{
  ...
  // UDP connection from N24 to N0
		  ...
		  // InetSocketAddress remote1 = InetSocketAddress (i.GetAddress (24),80);//, 0), 80);
		  // source1->Bind ();
		  // source1->Connect (remote1);
  Address sinkAddress1 (InetSocketAddress (i.GetAddress (0), sinkPort));
  PacketSinkHelper packetSinkHelper1 ("ns3::TcpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), sinkPort));
  ApplicationContainer sinkApps1 = packetSinkHelper1.Install (c.Get (0));
  Ptr<PacketSink> packetSink1 = sinkApps1.Get (0)->GetObject<PacketSink> ();
  packetSink1->TraceConnectWithoutContext ("Rx", MakeBoundCallback (&TraceSink, 0));
  sinkApps1.Start (Seconds (29.));
  sinkApps1.Stop (Seconds (90.));

  Ptr<Socket> ns3TcpSocket1 = Socket::CreateSocket (c.Get (24), TcpSocketFactory::GetTypeId ());
  Ptr<MyApp> app1 = CreateObject<MyApp> ();
  app1->Setup (ns3TcpSocket1, sinkAddress1, packetSize, numPackets, DataRate ("0.002Mbps"));
  c.Get (24)->AddApplication (app1);
  app1->SetStartTime (Seconds (30.));
  app1->SetStopTime (Seconds (90.));

  // UDP connection from N20 to N0
		  ...
		  // InetSocketAddress remote2 = InetSocketAddress (i.GetAddress (14),80);//, 0), 80);
		  // source2->Bind ();
		  // source2->Connect (remote2);
  // Address sinkAddress2 (InetSocketAddress (i.GetAddress (14), sinkPort));
  ...
  // Ptr<PacketSink> packetSink2 = sinkApps2.Get (0)->GetObject<PacketSink> ();
  // packetSink2->TraceConnectWithoutContext ("Rx", MakeBoundCallback (&TraceSink, 14));
  // sinkApps2.Start (Seconds (29.));
  // sinkApps2.Stop (Seconds (90.));

  Ptr<Socket> ns3TcpSocket2 = Socket::CreateSocket (c.Get (20), TcpSocketFactory::GetTypeId ());
  Ptr<MyApp> app2 = CreateObject<MyApp> ();
  app2->Setup (ns3TcpSocket2, sinkAddress1, packetSize, numPackets, DataRate ("0.002Mbps"));
  c.Get (20)->AddApplication (app2);
  app2->SetStartTime (Seconds (30.));
  app2->SetStopTime (Seconds (90.));

  // UDP connection from N4 to N0
		  ...
		  // InetSocketAddress remote3 = InetSocketAddress (i.GetAddress (4),80);//, 0), 80);
		  // source3->Bind ();
		  // source3->Connect (remote3);
  // Address sinkAddress3 (InetSocketAddress (i.GetAddress (4), sinkPort));
  ...
  // Ptr<PacketSink> packetSink3 = sinkApps3.Get (0)->GetObject<PacketSink> ();
  // packetSink3->TraceConnectWithoutContext ("Rx", MakeBoundCallback (&TraceSink, 4));
  // sinkApps3.Start (Seconds (29.));
  // sinkApps3.Stop (Seconds (90.));

  Ptr<Socket> ns3TcpSocket3 = Socket::CreateSocket (c.Get (4), TcpSocketFactory::GetTypeId ());
  Ptr<MyApp> app3 = CreateObject<MyApp> ();
  app3->Setup (ns3TcpSocket3, sinkAddress1, packetSize, numPackets, DataRate ("0.002Mbps"));
  c.Get (4)->AddApplication (app3);
  app3->SetStartTime (Seconds (30.));
  app3->SetStopTime (Seconds (90.));
